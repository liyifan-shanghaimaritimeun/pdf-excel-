...
old_extract_tables = '''    def extract_tables(self, page_num: int = 0) -> List[List[List[str]]]:
        """
        提取指定页面的表格数据
...
new_extract_tables = '''    def _save_page_image(self, page_num: int) -> str:
        """
        将PDF页面保存为图片到同名文件夹
...
content = content.replace(old_extract_tables, new_extract_tables)
